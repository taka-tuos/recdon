...
{
	struct json_object *content, *screen_name, *reblog;
	...
	char *sn = json_object_get_string(screen_name);
	...
	while(*src) {
		...
		if(!ltgt) {
			if(*src == '&') {
				...
				strcat_r(dst, *((unsigned char *)src));
			}
		}
		if(*src == '>') ltgt = 0;
		src++;
	...
	if(strcmp(head, "!rec") == 0) {
		char *len = strtok(NULL," ");
		char *tim = strtok(NULL," ");
		char *nam = strtok(NULL," ");
		
		printf("%s length, %s name=%s.\n",len,tim,nam);
...
int main(int argc, char *argv[])
{
	...
	if(fp) {
		...
		struct json_object *jobj_from_file = json_object_from_file(".token");
...
retry1:
		...
		do_oauth(code, ck, cs);
		struct json_object *token;
		jobj_from_file = json_object_from_file(".token");
		...
		sprintf(access_token, "Authorization: Bearer %s", json_object_get_string(token));
		printf("これでおしまい!\recdonライフを楽しんでね!\n");
	}
